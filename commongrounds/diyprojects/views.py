...
from .models import Favorite, Project, ProjectRating
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_authenticated:
            ...
            excluded_ids = (
                ...
                + list(
                    context["favorited_projects"].values_list(
                        "id", flat=True
                    )
                )
                + list(
                    context["reviewed_projects"].values_list(
                        "id", flat=True
                    )
                )
            )
...
class ProjectDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_authenticated:
            ...
            context["user_rating"] = (
                user_rating_obj.score if user_rating_obj else None
            )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if "submit_review" in request.POST:
            ...
            if favorite.exists():
                favorite.delete()
                messages.success(
                    request, f"You removed {project.title} from your favorites"
                )
            else:
                Favorite.objects.create(project=project, profile=profile)
                messages.success(
                    request, f"You added {project.title} to your favorites"
                )
